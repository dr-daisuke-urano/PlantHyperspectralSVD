...
This is the official implementation for Krishnamoorthi S et al. (2024) STAR*PROTOCOL. This protocol outlines the steps for correcting uneven lighting, identifying spectral components, and projecting hyperspectral cubes onto these components to highlight specific spectral features. Originally developed to analyze foliar color changes in Marchantia polymorpha under nutrient stress (Krishnamoorthi S et al. (2024) Cell Reports [https://doi.org/10.1016/j.celrep.2024.114463]), this STAR*PROTOCOL utilizes hyperspectral data in ornamental plants as alternative applications, focusing on distinguishing distinct foliar pigmentation patterns.

## Before your begin
Leaf color patterns in nature exhibit remarkable diversity, influenced by both genetic variation and environmental factors. Hyperspectral imaging captures leaf color patterns with high spectral resolution, unlike typical RGB cameras, which only capture three channels of blue, green, and red. This precise spectral resolution enables the detection of subtle color changes related to chemical properties (e.g., phytopigments like chlorophylls and anthocyanins) and structural leaf features. Hyperspectral images are stored as 3D cubes with spatial and spectral dimensions (x, y, and λ). With over 100 spectral channels, specialized analysis is needed to extract and visualize biologically meaningful data.

Spectral component analysis, also known as spectral decomposition or unmixing, is a powerful technique for extracting complex spectral patterns from leaf reflectance. By projecting hyperspectral images onto decomposed components, this method can reveal distinct color patterns, simplifying visualization of the high-dimensional hyperspectral cube, and often uncovering previously invisible features on leaves. 

The protocol below describes specific steps for applying spectral component analysis techniques, such as Singular Value Decomposition (SVD), Sparse Principal Component Analysis (SparsePCA), Non-negative Matrix Factorization (NMF), and Independent Component Analysis (ICA). Variations in pixel intensities, often caused by uneven lighting or complex leaf structures creating shadows, can hinder the identification of spectral components. To address this, the protocol includes instructions for setting up hyperspectral cameras and lighting, as well as preprocessing the hyperspectral cube data to reduce unevenness. 
